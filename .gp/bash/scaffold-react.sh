...
if [[ -n $(git status -s) ]]; then
  ...
  start_spinner "$msg"
  if git add -A && git commit -m "initial autogenerated commit" >/dev/null 2>&1; then
    stop_spinner 0
    git_committed=1
  else
    stop_spinner 1
...
if [[ -n $git_committed ]]; then
  _info_msg "Unstaged changes were committed to the local git repository using the message 'initial autogenerated commit"
