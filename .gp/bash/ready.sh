...
tasks_complete_msg() {
  green "Set up tasks are complete 🚀"
}

git_msg() {
  pink "If you are developing for the gp-react-on-rails-starter:"
  orange "  1. Before you push changes of your own revert the autogenerated commit, run:$c_blue git reset$c_end"
  orange "  2. Only commit and push your own changes by using the git aliases with the '-c' suffix!"
  pink "If you are developing a new project, before you commit changes of your own:"
  orange "  1. Push all autogenerated changes to the remote repository, run:$c_blue git new \"initial scaffolding\"$c_end"
}
...
main() {
  ...
  if [[ ! -f .gp/bash/locks/starter.lock ]]; then
    ...
    if [[ $(bash .gp/bash/helpers.sh is_inited) == 0 ]]; then
      ...
      pink "All Done"
      green "Start the development server with:$c_end$c_blue dserver start$c_end"
      green "The development server will automatically compile the client assets"
      green "Once client assets are compiled, open a preview with:$c_end$c_blue op hello_world$c_end"
      green "For more help opening a preview run:$c_end$c_blue op --help$c_end"
    else
      pink "Welcome back!"
      orange "The development server and preview browser have been closed"
      green "To start the development server again, run:$c_end$c_blue dserver start$c_end"
      green "To open the preview again run something like:$c_end$c_blue op hello_world$c_end"
      green "To check the status of the database, run:$c_end$c_blue pg_ctl status$c_end"
      green "To start the database, run:$c_end$c_blue pg_start$c_end"
      green "To stop the database, run:$c_end$c_blue pg_stop$c_end"
    fi
  fi
}
